...
function startFarmerAutoSell() {
  ...
  // Sunucuya doğduktan 10 saniye sonra ilk satışı yap
  setTimeout(() => {
    executeFarmerSell();
  }, 10000);

  // Her 10 dakikada bir (10 * 60 * 1000 ms) otomatik tekrar et
...
async function executeFarmerSell() {
  if (!bot || !bot.entity) {
    addLog('Bot oyunda olmadığı için çiftçi satışı ertelendi.');
    return;
  }

  addLog('/çiftçi komutu otomatik gönderiliyor...');
  bot.chat('/çiftçi');

  // 1. AŞAMA: İlk menü açılınca Zümrüt'e tıkla
  bot.once('windowOpen', async (window) => {
    addLog('Çiftçi menüsü açıldı, Zümrüt aranıyor...');

    // Menüdeki Zümrüt eşyasını bulur
...
function createBot() {
  ...
  bot.on('spawn', () => {
    ...
    // DÜZENLEME: Oyuna girer girmez çiftçi döngüsünü tamamen otomatik başlatır
    startFarmerAutoSell();
  });
...
app.post('/api/click', (req, res) => {
  const { type } = req.body;
  if (!bot || !bot.entity) return res.status(400).json({ success: false, message: 'Bot oyunda değil!' });

  try {
    if (type === 'left') {
      bot.swing('right');
      const targetEntity = bot.entityAtCursor(4);
      if (targetEntity) bot.attack(targetEntity);
    } else if (type === 'right') {
      const targetBlock = bot.blockAtCursor(4);
      if (targetBlock) bot.activateBlock(targetBlock);
...
app.post('/api/chat', (req, res) => {
  const { message } = req.body;
  if (!bot || !bot.entity) return res.status(400).json({ success: false });
  bot.chat(message);
  addChat(bot.username, message);
  res.json({ success: true });
});
...
app.post('/api/slot', (req, res) => {
  const { slot } = req.body;
  if (!bot || slot < 0 || slot > 8) return res.status(400).json({ success: false });
  bot.setQuickBarSlot(slot);
  botState.selectedSlot = slot;
  res.json({ success: true });
});
